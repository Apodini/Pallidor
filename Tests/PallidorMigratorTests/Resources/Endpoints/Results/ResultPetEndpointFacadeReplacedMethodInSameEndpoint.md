...
return PetAPI.updatePetWithForm(petId : outputDecoded.petId, name : outputDecoded.name, status : outputDecoded.status, authorization: authorization, contentType: contentType)
.mapError({( OpenAPIError($0 as? _OpenAPIError)! )})
.map({ (result) -> Pet in
    let context = JSContext()!
    context.evaluateScript("""
