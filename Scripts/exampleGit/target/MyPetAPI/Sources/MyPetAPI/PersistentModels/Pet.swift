...
public class Pet: Codable {
  ...
  init(
    category: Category?, id: Int64?, name: String, photoUrls: [String], status: Status?,
    tags: [Tag]?
  ) {
  ...
  func to() -> _Pet? {
    ...
    return _Pet(
      category: self.category?.to(), id: self.id, name: self.name, photoUrls: self.photoUrls,
      status: self.status?.to(), tags: self.tags?.map({ $0.to()! }))
  }
